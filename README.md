...
- **随机出题** — 单音、双音、三音和弦，音程多样（三度/四度/五度/六度），高音谱号 / 低音谱号
- **调号支持** — C、G、D、A、E、F、Bb、Eb 大调，自动渲染调号
- **临时变音** — 升降号与还原号随机出现
- **双模式回答** — 唱名（do re mi）或音名（C D E），变音后缀写法（do# / sib）
- **限时训练** — 可调倒计时（含进度条），设为 0 秒即不限时
- **自定义选题** — 可指定调号、谱号、音域、音名范围、音数、是否无尽模式
- **钢琴音反馈** — 主页面的声音按钮可开启（默认关闭），出题时自动播放当前音符
- **遮谱听音** — 设置里可开启“遮住谱面”：出题时只听音频，提交/超时显示答案时再显示谱面（开启时自动打开钢琴音）
